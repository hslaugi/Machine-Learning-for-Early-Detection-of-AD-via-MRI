...
from sklearn.metrics import classification_report  # <-- NEW: For per-class metrics
import numpy as np
import os
import warnings
import zipfile
...
# --- Model parameters ---
IMG_SIZE = (224, 224)
BATCH_SIZE = 128  # <-- UPDATED as requested
NUM_CLASSES = 4
EPOCHS = 50
...
try:
    ...
    # --- NEW: Per-Class Accuracy Report ---
    print("\n--- Per-Class Metrics (Test Set) ---")

    # Get all true labels from the test dataset
    y_true = np.concatenate([y for x, y in test_dataset], axis=0)

    # Get all predicted probabilities from the model
    y_pred_probs = best_model.predict(test_dataset)

    # Convert probabilities to class indices
    y_pred_classes = np.argmax(y_pred_probs, axis=1)

    # Use the class_names we saved from earlier
